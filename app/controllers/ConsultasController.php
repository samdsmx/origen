...
class ConsultasController extends BaseController {
	...
	public function getIndex() {
		...
		return View::make('sistemas.sistemas', array('menu' => $menu, "dentroPeriodo" => FALSE, 'periodos' => $text_select_periodo, 'ur_selected' => 0, 'opcionesPregunta' => $opcionesPregunta, 'sistemas' => $sistemas, "idPeriodoActual" => $periodo->id_periodo, 'posiblesResponsables' => $posiblesResponsables));
	}
	...
	public function getConsultasistemas($tipo, $period, $selectFiltro, $respuestaFiltro, $comparador) {
		...
		/* $sistemas = DB::select("select sasp.id_sistema, sasp.id_sistema_periodo,"
		  . " (select r.valor from sia_respuesta r where r.id_sistema_periodo = sasp.id_sistema_periodo and r.id_propiedad = 5) as sistema"
		  . " FROM sia_aso_sistema_periodo sasp "
		  . $condiciones
		  . " GROUP BY sasp.id_sistema"); */

		/* $sistemas = DB::select("select sasp.id_sistema as sistemaid, sasp.id_sistema_periodo, "
		  . "(select valor from sia_respuesta where id_sistema_periodo = sasp.id_sistema_periodo and id_propiedad = 5) as sistema, "
		  . "((select count(*) from sia_respuesta where id_sistema_periodo = sasp.id_sistema_periodo)/(select count(*) from sia_cat_propiedad where status = 1))*100 as estado, "
		  . "case when sasp.id_observacion = 4 then 'Baja' when "
...
//        return View::make('consultas', array('menu' => $menu, "sistemas" => $sistemas, "dentroPeriodo" => $dentroDelPeriodo, "idPeriodoActual" => $periodo->id_periodo));
	}
...
//				$excel->setCreator('Centro Nacional de Calculo');
//				$excel->sheet("sistema", function($sheet) use ($grupos, $fecha, $periodo, $sistema) {
//					$sheet->loadView('formato', array('propiedadesRes' => $grupos, 'fecha' => $fecha, 'autor' => Auth::user(), 'periodo' => $periodo, 'sistema' => $sistema));
//				});
//			});
//	}
