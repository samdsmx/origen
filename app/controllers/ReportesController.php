...
class ReportesController extends BaseController {
	...
	public function getIndex() {
		...
		$completos = DB::select('SELECT count(*) completos FROM sia_sistema s where status = 1 and (id_fase = 2 or id_fase = 4)');
		...
		$observaciones = DB::select('SELECT o.descripcion, count(*) cuenta
                                ...
                                where p.id_periodo = s.id_periodo and o.id_observacion = s.id_observacion and p.status = 1
		...
		$nombresCamposT = DB::select("SELECT column_name columna, table_name tabla "
				...
				. "WHERE table_schema = 'sia' and column_name != 'created_at' and column_name != 'status' and column_name != 'updated_at' and column_name not like 'id%' "
		...
		if ($tiempoFaltante[0]->tiempo >= 1) {
			$resultadoTareas = $incompletos[0]->incompletos / $tiempoFaltante[0]->tiempo;
		}
		...
		return View::make('reportes.reportes', array(
			    ...
			    'bajas' => $bajas[0]->bajas,
			    'completos' => $completos[0]->completos,
...
//			echo $observaciones[0];
	...
	public function postGraficap() {
		$tipotg = Input::get('tipotg');

		if (is_numeric($tipotg)) {
			if ($tipotg == 2) {
				$periodo = Input::get('periodo');
				$comparado = Input::get('comparado');
				$idGrafica = Input::get('idgrafica');
				...
				foreach ($elementoDividido as $el) {
					...
					if ($tabla == 'sia_observacion') {
						...
						if ($filtro != '0') {
							$busqueda = DB::select('select count(*) cuenta from sia_aso_sistema_periodo sasp join sia_sistema ss on sasp.id_sistema = ss.id_sistema join sia_periodo sp on sasp.id_periodo = sp.id_periodo join sia_respuesta sr on sr.id_sistema_periodo = sasp.id_sistema_periodo join sia_cat_propiedad t on sr.id_propiedad = t.id_propiedad  where ' . $condiciones);
							$resultado[] = $campo . ' ' . $filtro . ' ' . $filtroc . ':' . $busqueda[0]->cuenta;
						} else {
							$busquedaTem = DB::select('select ' . $campo . ' buscador from ' . $tabla);
							foreach ($busquedaTem as $bt) {
								$busqueda = DB::select('select count(*) cuenta from sia_aso_sistema_periodo sasp join sia_sistema ss on sasp.id_sistema = ss.id_sistema join sia_periodo sp on sasp.id_periodo = sp.id_periodo join sia_respuesta sr on sr.id_sistema_periodo = sasp.id_sistema_periodo join sia_cat_propiedad t on sr.id_propiedad = t.id_propiedad where t.' . $campo . '="' . $bt->buscador . '"' . $condiciones);
								$resultado[] = $bt->buscador . ':' . $busqueda[0]->cuenta;
							}
						}
					} else if ($tabla == 'sia_respuesta') {
						if ($filtro != '0') {
							$busqueda = DB::select('select count(*) cuenta from sia_aso_sistema_periodo sasp join sia_sistema ss on sasp.id_sistema = ss.id_sistema join sia_periodo sp on sasp.id_periodo = sp.id_periodo join sia_respuesta t on sasp.id_sistema_periodo = t.id_sistema_periodo where ' . $condiciones);
							$resultado[] = $campo . ' ' . $filtro . ' ' . $filtroc . ':' . $busqueda[0]->cuenta;
						} else {
							$busquedaTem = DB::select('select ' . $campo . ' buscador from ' . $tabla);
							foreach ($busquedaTem as $bt) {
								$busqueda = DB::select('select count(*) cuenta from sia_aso_sistema_periodo sasp join sia_sistema ss on sasp.id_sistema = ss.id_sistema join sia_periodo sp on sasp.id_periodo = sp.id_periodo join sia_respuesta t on sasp.id_sistema_periodo = t.id_sistema_periodo where t.' . $campo . '="' . $bt->buscador . '"' . $condiciones);
								$resultado[] = $bt->buscador . ':' . $busqueda[0]->cuenta;
							}
						}
					}
	...
	public
		function postExcel() {
		$informacionTabulada = Input::get('informacionTabulada');
		$fechasTabulada = Input::get('fechasTabuladas');
		$fechasDivididas = explode('%', $fechasTabulada);
//		$fechasDivididas = array('Hola','Adios');
