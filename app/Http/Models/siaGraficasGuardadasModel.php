...
class siaGraficasGuardadasModel extends Model{
	...
	public static function getInfoElementos($idGrafica) {
		$elementosBase = array
		    ...
		    array(5, 1, 'Migración', 'select count(*) cuenta FROM sia_aso_sistema_periodo s left join sia_observacion o on s.id_observacion = o.id_observacion left join sia_periodo p on s.id_periodo = p.id_periodo', 'p.status = 1 and o.descripcion = "Migracion"'),
		    array(6, 2, 'Incompletos', 'select count(*) cuenta from sia_sistema ss left join sia_aso_sistema_periodo sasp on ss.id_sistema = sasp.id_sistema left join sia_periodo sp on sp.id_periodo = sasp.id_periodo ', 'ss.status = 1 and (ss.id_fase = 1 or ss.id_fase = 3)'),
		    array(7, 2, 'Completos', 'select count(*) cuenta from sia_sistema ss left join sia_aso_sistema_periodo sasp on ss.id_sistema = sasp.id_sistema left join sia_periodo sp on sp.id_periodo = sasp.id_periodo ', 'ss.status = 1 and (ss.id_fase = 2 or ss.id_fase = 4)')
		);
		$resultado = array();
