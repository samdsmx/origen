...
class clsTemplate
{
  ...
  function LoadTemplate($filename, $block_name, $encoding = "", $meta = "")
  {
    ...
    if (file_exists($file_path))
    {
      ...
      fclose($fh);
      if ($meta == "remove") {
        $file_content =  preg_replace("/<\\s*meta\\s+http-equiv\\s*=\\s*['\"]?content-type['\"]?\\s+content\\s*=\\s*['\"]?text\\/html;\\s*charset=[-a-z0-9]+['\"]?[^>]*>/i", "", $file_content);
        $file_content =  preg_replace("/<\\s*meta\\s+content\\s*=\\s*['\"]?text\\/html;\\s*charset=[-a-z0-9]+['\"]?\\s+http-equiv\\s*=\\s*['\"]?content-type['\"]?[^>]*>/i", "", $file_content);
      } else if ($meta == "replace") {
        $file_content =  preg_replace("/(<\\s*meta\\s+http-equiv\\s*=\\s*['\"]?content-type['\"]?\\s+content\\s*=\\s*['\"]?text\\/html;\\s*charset=)([-a-z0-9]+)(['\"]?[^>]*>)/i", "\\1" . $CCSLocales->GetFormatInfo("Encoding") . "\\3", $file_content);
        $file_content =  preg_replace("/(<\\s*meta\\s+content\\s*=\\s*['\"]?text\\/html;\\s*charset=)([-a-z0-9]+)(['\"]?\\s+http-equiv\\s*=\\s*['\"]?content-type['\"]?[^>]*>)/i", "\\1" . $CCSLocales->GetFormatInfo("Encoding") . "\\3", $file_content);
      }
