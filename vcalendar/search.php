<?php
//Include Common Files @1-97A2E2E5
define("RelativePath", ".");
...
//Class_Initialize Event @9-6586386D
...
"FROM categories_langs {SQL_Where} {SQL_OrderBy}";
            ...
            $this->s_event_date_from = & new clsControl(ccsTextBox, "s_event_date_from", $CCSLocales->GetText("event_date") . " " . $CCSLocales->GetText("from"), ccsDate, array("ShortDate"), CCGetRequestParam("s_event_date_from", $Method), $this);
            $this->DatePicker_s_event_date_from = & new clsDatePicker("DatePicker_s_event_date_from", "events_groupsSearch", "s_event_date_from", $this);
            $this->s_event_date_to = & new clsControl(ccsTextBox, "s_event_date_to", $CCSLocales->GetText("event_date") . " " . $CCSLocales->GetText("to"), ccsDate, array("ShortDate"), CCGetRequestParam("s_event_date_to", $Method), $this);
            $this->DatePicker_s_event_date_to = & new clsDatePicker("DatePicker_s_event_date_to", "events_groupsSearch", "s_event_date_to", $this);
            $this->Button_DoSearch = & new clsButton("Button_DoSearch", $Method, $this);
        }
    }
//End Class_Initialize Event

//Validate Method @9-FC5C5453
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        ...
        $Validation = ($this->s_event_date_from->Validate() && $Validation);
        $Validation = ($this->s_event_date_to->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        $Validation =  $Validation && ($this->s_keyword->Errors->Count() == 0);
        $Validation =  $Validation && ($this->s_category->Errors->Count() == 0);
        $Validation =  $Validation && ($this->s_event_date_from->Errors->Count() == 0);
        $Validation =  $Validation && ($this->s_event_date_to->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @9-72F085D9
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->s_keyword->Errors->Count());
        $errors = ($errors || $this->s_category->Errors->Count());
        $errors = ($errors || $this->s_event_date_from->Errors->Count());
        $errors = ($errors || $this->DatePicker_s_event_date_from->Errors->Count());
        $errors = ($errors || $this->s_event_date_to->Errors->Count());
        $errors = ($errors || $this->DatePicker_s_event_date_to->Errors->Count());
        $errors = ($errors || $this->Errors->Count());
        return $errors;
    }
//End CheckErrors Method

//Operation Method @9-BA8B327E
    function Operation()
    {
        if(!$this->Visible)
            return;
        ...
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "search.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Show Method @9-25285E14
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->s_event_date_from->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DatePicker_s_event_date_from->Errors->ToString());
            $Error = ComposeStrings($Error, $this->s_event_date_to->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DatePicker_s_event_date_to->Errors->ToString());
        ...
        $this->s_event_date_from->Show();
        $this->DatePicker_s_event_date_from->Show();
        $this->s_event_date_to->Show();
        $this->DatePicker_s_event_date_to->Show();
...
//Class_Initialize Event @5-2DDA44A4
...
//Show Method @5-FFB1AA17
    function Show()
    {
        ...
        $this->DataSource->Parameters["urls_event_date_to"] = CCGetFromGet("s_event_date_to", "");
        $this->DataSource->Parameters["urls_event_date_from"] = CCGetFromGet("s_event_date_from", "");
...
//Prepare Method @5-0FECB47C
    function Prepare()
    {
        ...
        $this->wp->AddParameter("6", "urls_event_date_to", ccsDate, $DefaultDateFormat, array("yyyy", "-", "mm", "-", "dd", " ", "HH", ":", "nn", ":", "ss"), $this->Parameters["urls_event_date_to"], "", false);
        $this->wp->AddParameter("7", "urls_event_date_from", ccsDate, $DefaultDateFormat, array("yyyy", "-", "mm", "-", "dd", " ", "HH", ":", "nn", ":", "ss"), $this->Parameters["urls_event_date_from"], "", false);
        ...
        $this->wp->Criterion[6] = $this->wp->Operation(opLessThanOrEqual, "events.event_date", $this->wp->GetDBValue("6"), $this->ToSQL($this->wp->GetDBValue("6"), ccsDate),false);
...
//Open Method @5-553B3DAE
    function Open()
    {
        ...
        $this->SQL = "SELECT category_name, event_title, event_date, event_time, event_time_end, event_id  " .
        "FROM events LEFT JOIN categories_langs ON " .
...
//Include events file @1-659EE2D8
include("./search_events.php");
//End Include events file

//Initialize Objects @1-892BBE19
...
//Go to destination page @1-96119EE0
if($Redirect)
{
